# Mesh 網路技術簡報（軍事應用導向）

---

## 一、名詞解釋

### 1.1 網路拓撲類型

| 拓撲 | 結構 | 特點 | 軍事場景舉例 |
|------|------|------|------|
| **星狀（Star）** | 所有節點連到單一中心 | 中心掛 = 全斷，佈線簡單 | 傳統 Wi-Fi：一台 router 服務全連部 |
| **匯流排（Bus）** | 所有節點共用一條主線 | 主線斷 = 全斷，省線材 | 舊式軍用有線電話（field wire）沿戰壕佈一條線 |
| **環狀（Ring）** | 節點串成閉環 | 任一節點掛，還可反向繞，但兩處斷 = 癱瘓 | 軍用光纖骨幹環（SONET/SDH 保護環） |
| **網狀（Mesh）** | 每個節點互連，多條冗餘路徑 | 單點失效無影響，自癒繞路，但節點成本高 | **本簡報主題**——戰術無線電自組網 |

### 1.2 Mesh 核心概念

| 名詞 | 定義 |
|------|------|
| **Mesh Network** | 網狀網路。每個節點同時是終端也是中繼，拓撲為 partial/full mesh，具自組網、自癒、多跳路由三特性 |
| **Multi-hop Routing** | 多跳路由。封包經多個中間節點轉送到目的地，每跳增加延遲但延伸覆蓋範圍 |
| **Self-Healing** | 自癒。任一節點失效或鏈路中斷時，網路自動重新計算路由，繞開故障點 |
| **Ad-hoc Network** | 隨建即連網路。無固定基礎設施，節點動態加入/離開，拓撲持續變化 |
| **MANET** | Mobile Ad-hoc Network，移動中的 Ad-hoc 網路，詳見第三節 |

### 1.3 常用縮寫

| 縮寫 | 全稱 | 意思 |
|------|------|------|
| **PHY** | Physical Layer | 實體層，OSI 模型第一層，定義無線電的頻率、調變、編碼方式。LoRa 和 Wi-Fi 是不同的 PHY |
| **COTS** | Commercial Off-The-Shelf | 商用現貨。不需要軍規認證就能買到的現成產品，例如 goTenna、OpenWrt 路由器 |

---

## 二、現有 Mesh 系統

### 2.1 戰術無線電 — TrellisWare TSM

**TrellisWare** 是美軍戰術 Mesh 通訊的核心技術供應商，其 **TSM（Tactical Scalable MANET）** 波形為 Nett Warrior 單兵系統的標準通訊層。

| 項目 | 規格 |
|------|------|
| **開發商** | TrellisWare Technologies（美） |
| **波形** | TSM（Tactical Scalable MANET） |
| **頻段** | UHF 225-450 MHz / L-band 1.2-1.4 GHz |
| **速率** | 窄頻模式 ~250 kbps，寬頻模式最高 **10 Mbps** |
| **節點規模** | 單一 MANET 支援 **200+ 節點** |
| **每跳延遲** | <1 ms |
| **自癒時間** | <1 秒 |
| **部署平台** | AN/PRC-163 (Falcon IV)、AN/PRC-117G (Falcon III) |

**核心優勢**：同一波形同時承載語音、數據、位置、影片，所有節點自動中繼。失去任一節點不影響網路，自癒時間 <1 秒。已在伊拉克、阿富汗實戰驗證。

### 2.2 Wi-Fi Mesh — IEEE 802.11s

一般消費级 Mesh Wi-Fi（如 Deco、Orbi）是在 802.11s 標準之上做各家私有擴充。802.11s 的核心是一個標準化的 Mesh 路由協議 HWMP，讓不同廠牌的 AP 可以互相中繼。

| 項目 | 說明 |
|------|------|
| **標準** | IEEE 802.11s（Wi-Fi Mesh 國際標準） |
| **頻段** | 2.4 / 5 GHz |
| **速率** | 視 802.11 世代，最高可達數百 Mbps |
| **距離** | 單跳 100-300m（室內更短） |
| **路由協議** | HWMP（Hybrid Wireless Mesh Protocol） |

**HWMP** 混合 AODV（on-demand）和 tree-based（proactive）兩種模式，適應固定與移動節點混合場景。適合前進基地或固守陣地內部的高頻寬覆蓋，但不適合長距離或高機動場景。

### 2.3 Meshtastic

Meshtastic 是開源社群開發的 LoRa Mesh 通訊系統，設計目標是「沒有手機訊號也能傳文字訊息」。硬體便宜（$30 一顆）、開源、吃 LoRa 長距特性，是民間 prepper 和山區通訊社群的主流方案。

| 項目 | 規格 |
|------|------|
| **硬體** | LoRa SX1276/SX1262 晶片（Semtech） |
| **頻段** | 868 MHz (EU) / 915 MHz (US) / 470 MHz (CN) |
| **速率** | ~1 kbps（LoRa 長距模式的代價） |
| **範圍** | 空曠地 10-20 km 單跳，城市 1-3 km |
| **路由** | 泛洪式（flooding），非真正路由協議 |

Meshtastic 的軍事價值不在直接採用，而在證明了 **LoRa PHY + Mesh topology = 極低成本的大範圍文字通訊網**。缺點是 flooding 路由在大規模時頻譜效率極差。

### 2.4 其他 Mesh 相關系統與協議

| 系統 | 類型 | 用途 |
|------|------|------|
| **GoTenna** | 消費級戰術通訊 | VHF 151-154 MHz，手機藍牙連線，文字+GPS，點對點 6-8 km，特戰或山區無基地台場景 |
| **Silvus StreamCaster** | 軍用 COTS MANET | MIMO 波形，400+ Mbps，用於 ISR 影片回傳等高頻寬戰術需求 |
| **Thread** | IoT Mesh | Google/Nest 主導，6LoWPAN，每個裝置都是 router，用於智慧家庭骨幹 |

---

## 三、MANET（Mobile Ad-hoc Network）

### 3.1 定義

> MANET = 一群移動中的節點，不依賴任何固定基礎設施，各自獨立移動，透過無線鏈路動態形成臨時網路。每個節點同時是 host 和 router。

與固定 Mesh 的關鍵差異：**節點移動**導致拓撲持續變化、鏈路頻繁斷連重建、路由協議必須是反應式或低延遲主動式。

### 3.2 路由協議分類

| 類型 | 代表協議 | 機制 | 適用 |
|------|---------|------|------|
| **Proactive（主動式）** | OLSR, DSDV | 定期交換路由表，拓撲常保最新 | 節點數少、移動慢 |
| **Reactive（反應式）** | AODV, DSR | 有需求才發 RREQ 找路 | 節點多、移動快 |
| **Hybrid（混合式）** | ZRP, HWMP | 區域內 proactive、跨區 reactive | 大規模分層網路 |
| **Geographic（地理式）** | GPSR, LAR | 靠 GPS 座標決定下一跳，不維護路由表 | 高機動、大規模 |

---

## 四、作戰運用

### 4.1 戰術層級 × 硬體 × 場景對應

| 層級 | 通訊需求 | 硬體 | 適用場景 |
|------|---------|---------|---------|
| **班/排級** | 語音 + 位置（< 2 km） | AN/PRC-163 手持（5W，鞭狀天線） | 城鎮戰：每員一個節點，建築物內多跳繞障礙，自動中繼 |
| **連級** | 語音 + 數據 + 影片（< 10 km） | AN/PRC-163 車載配置（20W，車頂天線）+ UAV 中繼 | 機動突擊：車隊 >40 km/h 移動中保持 MANET，脫隊自動重組 |
| **營級** | 態勢圖 + 火力協調（< 40 km） | AN/PRC-117G 車載（20-50W，車頂/桅杆天線）+ SATCOM 上行 | 營指揮所：Mesh 骨幹內部通聯（車載台互連 10-40 km），SATCOM 往上回報 |
| **特戰滲透** | 文字 + GPS（< 8 km） | goTenna | 小隊深入敵後：低功率難偵測，無需基地台，文字+位置夠用 |
| **前進基地** | 高頻寬內部通訊（< 300m） | OpenWrt + 802.11s | 固守陣地：$50 一顆可拋棄式，VoIP + 影片監控，被炸不心疼 |
| **戰場感測器網** | 低速率回傳（3-8 km 單跳） | Heltec LoRa32 V3 | 沿補給線佈放：震動/磁力感測，車輛經過自動回傳，太陽能供電 |
| **旅級以上** | 大量數據 + 跨軍種（>40 km，跨戰區） | SATCOM 終端（AN/TSC-167 或 VSAT）為骨幹；AN/PRC-117G 車載 Mesh 負責指揮所周邊最後一哩基地台接取 | 旅指揮所：骨幹走衛星/光纖跨戰區回傳，營級 Mesh 骨幹的車載台匯聚到旅級後，由旅級 SATCOM 統一往上報 |

### 4.2 美軍 Nett Warrior 案例

Nett Warrior 是美軍現役單兵系統，核心通訊層使用 TrellisWare TSM 波形：

- 每班 9-13 人各持 AN/PRC-163，自動形成 MANET
- 班長終端顯示全隊即時位置（無 GPS 時靠慣性導航 + Mesh 相對定位）
- 語音走 mesh 中繼，不需排級中繼台
- 失去任何 1-2 員不影響網路，自癒時間 <1 秒

---

## 五、總結

為什麼戰場上需要 Mesh？因為三個戰場現實，傳統星狀網路全部無法應付：

1. **單點失效是日常**。基地台、指揮車、排級中繼台都是優先被打的目標。星狀網路中心一倒，整個通訊網跟著癱瘓。Mesh 沒有中心——每個節點都是中繼，打掉任何一個，訊號自動繞別條路。

2. **節點頻繁移動**。步兵徒步、車隊機動、UAV 盤旋——節點位置每一秒都在變。傳統網路需要預先規劃路由，Mesh 的 ad-hoc 特性讓節點動態加入/離開，拓撲即時重組。

3. **沒有基礎設施可依賴**。深入敵後沒有基地台、灘岸第一線光纖早被炸斷、災區基礎通訊全毀。Mesh 只需要節點本身通電，就可以自成網路。不需要任何外部設施。

Mesh / MANET 的軍事價值不在於取代 SATCOM 或傳統戰鬥無線電，而在於填補兩者之間的空白：SATCOM 太高（延遲）、傳統無線電太僵（單點失效）、Mesh/MANET 剛好填了中距離高機動的抗毀通訊需求。

未來方向：低軌衛星（Starlink/OneWeb）已具備星間鏈路（ISL），本質上就是太空中的 Mesh。地面 MANET + 空中 Mesh（UAV）+ 太空 Mesh（LEO ISL）的三層整合，會是下一代戰術通訊的核心架構。
